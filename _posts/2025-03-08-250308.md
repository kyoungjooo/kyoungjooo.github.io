...
기본적으로 리엑트는 컴포넌트 함수 내부의 코드는 처음 렌더링될 때 한 번 실행되며, <span class="color_yellow">일반적인 변수는 값이 변경되어도 리액트의 상태를 관리할 수 없고, useState 를 사용해야 UI를 업데이트를 할 수 있다.</span>
...
따라서 아래의 결과값이 기대값과 일치하지 않는 이유를 설명하면! <br><span class="color_yellow">setState 는 비동기적으로 실행되기때문에 state 값이 즉시 업데이트되지 않고, 변경이 반영되지 않은 state를 직접 사용해 상태를 변경을 호출하면 초기값을 기준으로 계산</span>되기 때문에 예상치 못한 결과가 나오는 것 !
